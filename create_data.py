...
for char in string.ascii_uppercase:			#Iterate through A..Z
	...
	for count in range(1, 21):						#For each letter go through 1 to 20
		...
		#imNew = Image.new("L", (10,10))			#Create new grayscale 10x10 image
		#imNewP = imNew.load()								#Load image to manipulate pixels
		imgArr= []													#Array to hold the 100 avg pixel values
		
		for x in range(0,10):				#Go through all rows in 10x10 image
			for y in range(0, 10):		#Go through all columns in 10x10 image
				arr = None							#Reset value array
				arr = [									#Create array from pixel values of the 3x3 subset area.
				...
				#imNewP[x, y] = sum(arr)/len(arr)					#Insert avg pixel val to image
